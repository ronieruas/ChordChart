...
# --- SONGS API ROUTES (UPDATED FOR SORTING) ---
@app.route('/api/songs', methods=['GET'])
@login_required
def get_songs_filtered():
    """Fetches songs based on a filter, always sorted alphabetically."""
    ...
    if filter_type == 'public':
        query = 'SELECT id, title, original_key, user_id FROM songs WHERE is_public = 1 ORDER BY title ASC'
    else: # Default to 'my_songs'
        query = 'SELECT id, title, original_key, user_id FROM songs WHERE user_id = ? ORDER BY title ASC'
